...
func get_attack_animation() -> String:
	# Determinar a direção do ataque com base no vetor de input
	if input_vector.y < 0:
		return "attack_up_1" if not sprite.flip_h else "attack_up_2";
	elif input_vector.y > 0:
		return "attack_down_1" if not sprite.flip_h else "attack_down_2";
	elif input_vector.x < 0:
		return "attack_side_2";
	elif input_vector.x > 0:
		return "attack_side_1";
	else:
	# Se não houver movimento, retornar a última direção conhecida
		return "attack_side_2" if sprite.flip_h else "attack_side_1";

func attack() -> void:
	if is_attacking: return;
	
	# Determinar a direção e a variação do ataque
	...
	for body in bodies:
		if body.is_in_group("enemies"):
			...
			if input_vector.y < 0:
				...
				# Se não houver movimento, determine a direção com base no movimento do sprite
